...
type Refund struct {
	...
	PaymentId            string      `json:"paymentId"            ` // 支付单号(内部生成，支付单号）
	RefundId             string      `json:"refundId"             ` // 退款单号。可以唯一代表一笔退款（内部生成，退款单号）
	RefundFee            int64       `json:"refundFee"            ` // 退款金额。单位：分
	RefundComment        string      `json:"refundComment"        ` // 退款备注
	Status               int         `json:"status"               ` // 退款状态。10-退款中，20-退款成功，30-退款失败
	...
	ChannelRefundId      string      `json:"channelRefundId"      ` // 外部退款单号

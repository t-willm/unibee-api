...
func SubscriptionPlanChannelTransferAndActivate(ctx context.Context, planId int64, channelId int64) error {
	...
	if len(planChannel.ChannelProductId) == 0 {
		...
		res, err := channel.GetPayChannelServiceProvider(ctx, int64(payChannel.Id)).DoRemoteChannelProductCreate(ctx, plan, planChannel)
		if err != nil {
			return err
		}
		//更新 planChannel
	...
	if len(planChannel.ChannelPlanId) == 0 {
		//创建 并激活 Plan
		res, err := channel.GetPayChannelServiceProvider(ctx, int64(payChannel.Id)).DoRemoteChannelPlanCreateAndActivate(ctx, plan, planChannel)
		if err != nil {
			return err
		}
		_, err = dao.SubscriptionPlanChannel.Ctx(ctx).Data(g.Map{

...
func (c *Client) CreateOrder(ctx context.Context, intent string, purchaseUnits []PurchaseUnitRequest, payer *CreateOrderPayer, paymentSource *PaymentSource, appContext *ApplicationContext, paypalRequestId string) (*Order, error) {
	return c.CreateOrderWithPaypalRequestID(ctx, intent, purchaseUnits, payer, paymentSource, appContext, paypalRequestId)
}
...
func (c *Client) CreateOrderWithPaypalRequestID(ctx context.Context,
	...
	req, err := c.NewRequest(ctx, "POST", fmt.Sprintf("%s%s", c.APIBase, "/v2/checkout/orders"), createOrderRequest{Intent: intent, PurchaseUnits: purchaseUnits, Payer: payer, PaymentSource: paymentSource, ApplicationContext: appContext})
	if err != nil {
		return order, err
	}

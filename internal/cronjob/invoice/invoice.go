...
func TaskForExpireInvoices(ctx context.Context) {
	...
	for _, one := range list {
		...
		if utility.TryLock(ctx, key, 60) {
			...
			} else if one.FinishTime+(one.DayUtilDue*86400)+1800 < gtime.Now().Timestamp() { // task delay half an hour to expire
				//Invoice Expire
...
func ExpireUserSubInvoices(ctx context.Context, sub *entity.Subscription, timeNow int64) {
	...
	for _, one := range list {
		...
		if utility.TryLock(ctx, key, 60) {
			...
			} else if one.FinishTime+(one.DayUtilDue*86400)+1800 < timeNow {
				//Invoice Expire

...
import (
	...
	"go-oversea-pay/internal/logic/channel/webhook"
...
func ChannelPaymentWebhookEntrance(r *ghttp.Request) {
	...
	webhook.GetPayChannelWebhookServiceProvider(r.Context(), int64(channelIdInt)).DoRemoteChannelWebhook(r, payChannel)
}
...
func ChannelPaymentRedirectEntrance(r *ghttp.Request) {
	...
	payChannel := util.GetOverseaPayChannel(r.Context(), int64(channelIdInt))
	redirect, err := webhook.GetPayChannelWebhookServiceProvider(r.Context(), int64(channelIdInt)).DoRemoteChannelRedirect(r, payChannel)
	if err != nil {
